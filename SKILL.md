...
- `./run check` — cwd = the worktree being gated. Build + lint + FULL tests vs a
  ...
  **5-minute soft budget, 10-minute hard timeout** (no config/CLI override).
  ...
  and `last-green` complete and the gate lock is released, Greenline asks AgentD3
  to enqueue an optimization task for that repo/SHA/duration. AgentD3 failure is
  loud and journaled but best-effort. Beyond 10 minutes the whole check process
  group is killed and the gate fails, with no task. Speed the suite up (parallelize with isolated
...
- **The 5-minute check budget is soft; 10 minutes is the non-configurable hard timeout.** A successful 5–10 minute check lands normally, then Greenline enqueues an AgentD3 optimization task after releasing the gate lock. AgentD3 failure never rolls back healthy work. Beyond 10 minutes Greenline kills the whole check process group and fails without creating a task. Budget before moving tests onto a live provider: record cassettes first, and make anything the prompt embeds deterministic (wall-clock time and weather in a rendered prompt change the cache key every run, so cassettes never replay and every run pays full inference).
