...
five-minute budget and safe to run concurrently from multiple worktrees.
...
- **The 5-minute check cap is doctrine, not config (`CHECK_TIMEOUT_SECONDS = 300`, "a repo whose check outgrows this makes the suite faster, it does not raise the cap").** A suite that starts exercising real inference per scenario will drift into it and then fail with `CHECK TIMED OUT`, which looks nothing like a red test. Budget for it BEFORE moving tests onto a live provider: record cassettes first, and make anything the prompt embeds deterministic (wall-clock time and weather in a rendered prompt change the cache key every run, so cassettes never replay and every run pays full inference).
